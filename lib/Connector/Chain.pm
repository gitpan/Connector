# Connector::Chain
#
# Connector class capable of dealing with fallback to additional connectors
#
# Written by Scott Hardin and Martin Bartosch for the OpenXPKI project 2012
#
package Connector::Chain;
...
sub _route_call {
    ...
    if ( ! $conn ) {
        die "ERR: no default connector for Connector::Chain";
    }
    ...
    $self->log()->debug('Call '.$call.' in Chain to '. join('.', @suffix));
        ...
        if ( $meta && $meta->{TYPE} eq 'reference' ) {
            if (  $meta->{VALUE} =~ m/^([^:]+):(.+)$/ ) {
                ...
                if ( $schema eq 'connector' ) {
                    ...
                    if ( ! $conn ) {
                        die "Connector::Chain: error creating connector for '$target': $@";
                    }
                    # Push path on top of the argument array
                    unshift @args, join($delim, @suffix); 
                    return $conn->$call( @args );                    
                } else {
                    die "Connector::Chain: unsupported schema for symlink: $schema";
                }
            } else {
                # redirect
...
source that Chain accesses for get() requests. If the request returns a reference
to a SCALAR, Chain interprets this as a symbolic link. The content of the 
...
With the symlink now in the key, Chain must walk down each level itself and 
...
This is a reference to the Connector instance that Connector::Chain
